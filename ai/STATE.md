...
- [x] **Week 1 spec written 2026-09-22: docs/WEEK1-SPEC.md.** Names every exported type and test file for the single-screen recorder so `/orchestrate` doesn't have to invent them: `SessionMachine` (pure reducer, no AVFoundation import), `CaptureService`/`MockCaptureService`/`AVCaptureService` (actor-based, Swift 6 strict concurrency), `SegmentLedger` (crash-recovery ledger), deck data (`Deck`/`Question`/`Decks`), `ExportManifest`. Full acceptance table (20 rows -> `StoryCueTests/SessionMachineTests.swift` test methods) built from Sol's event table plus PLAN section 4 item 5's "deck index, timer, export manifest" line. Deck copy is explicitly scoped OUT (placeholder `// TODO copy` text; real copy is a later Routine-tier pass). Nil-safety-at-init and the Duo-line exclusion (no direction-coordinator code in this week's `AVCaptureService`) are named as spec requirements, not implementation details, to avoid CI failing for reasons unrelated to the state machine.
...
- [ ] **Week 1 implementation via `/orchestrate`: spec is ready (docs/WEEK1-SPEC.md), dispatch not yet run.** Next session: `/orchestrate` the spec (Kimi executor implements `SessionMachine`/`CaptureService`/`SegmentLedger`/deck data against the named types and acceptance table; Sol audits the diff). Then Kimi state-machine enumeration (below) once `SessionMachine` exists, then Sol reviews the recorder diff at the week-1 gate (plan section 9).
...
## Git state
- Branch: main, remote https://github.com/pmartin1915/storycue.git (private). 2026-09-21 session: b8e5902 (synthesis + Sol review + PLAN pass), 195da92 (scaffold), 4d99fe3 + 332af19 (handoff, STATE, build.yml concurrency). 2026-09-22 session: 994e577 (runs-on label fix), 84378e7 (PRODUCT_NAME fix), 30696c0 (ENABLE_TESTABILITY fix) -- CI green on 30696c0 (run 35676600490). 2026-09-22 session (spec pass): docs/WEEK1-SPEC.md + this STATE update + docs/HANDOFF-2026-09-22-week1-spec.md, committed on top of 30696c0.
